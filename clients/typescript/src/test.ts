...
async function main() {
    const client = new Client('127.0.0.1', 6380);
    ...
    try {
        ...
        console.log(`Got: ${val}`);
        if (val !== 'world_ts') throw new Error('Value mismatch');
        ...
        if (!results.includes('v_ts')) throw new Error('v_ts not found in results');
